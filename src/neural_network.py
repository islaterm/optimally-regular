...
class FFNN(torch.nn.Module):
    ...
    def __init__(self, features: int, hidden_layers: List[int],
        ...
        if ac_fn_arguments is not None:
            self.__fn_args_mask = [Parameter(torch.tensor(p)) if p else None for p in
                                   ac_fn_arguments]
        else:
            self.__fn_args_mask = [None for _ in activation_functions]
        self.__cache = []
    ...
    def forward(self, inputs: Tensor) -> Tensor:
        ...
        for weight, bias, ac_function, args in zip(self.__weights[:-1], self.__biases[:-1],
            ...
            prediction = ac_function(prediction, args.item()) if args else ac_function(prediction)
