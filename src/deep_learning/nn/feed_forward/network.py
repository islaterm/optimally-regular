...
from deep_learning.functions.activation import ActivationFunction, derivatives, softmax
...
class FFNN(torch.nn.Module):
    ...
    def __init__(self,
        ...
        self.__keep_prob = [] if keep_prob is None else [
            torch.ge(-1 * r, -1 * p) for r, p in zip(rand_matrix, keep_prob)
        ]
        self.__cache = []

    def __setup_hyperparams(self, features: int, hidden_layers: List[int],
                            classes: int):
        sizes = [features] + hidden_layers + [classes]
        self.__weights = ParameterList([
            get_init_weights((sizes[i], sizes[i + 1]))
            for i in range(len(sizes) - 1)
        ])
        self.__biases = ParameterList(
            [Parameter(torch.zeros(h)) for h in sizes[1:]])

    def __setup_ac_funs(self, activation_functions: List[ActivationFunction],
                        ac_fn_arguments):
    ...
    def forward(self,
        ...
        prediction = inputs
        if not predict:
            for weight, bias, ac_function, args, prob in zip(
                    self.__weights[:-1], self.__biases[:-1],
                    self.__ac_functions, self.__fn_args_mask,
                    self.__keep_prob[:-1]):
                prob = prob.to(device)
                prediction = torch.mm(prediction, weight * prob) + bias
                self.__cache.append(prediction)
                prediction = ac_function(
                    prediction,
                    args.item()) if args else ac_function(prediction)
            return softmax(
                torch.mm(prediction, self.__weights[-1] *
                         self.__keep_prob[-1].to(device)) + self.__biases[-1],
                dim=1)
        for weight, bias, ac_function, args in zip(self.__weights[:-1],
                                                   self.__biases[:-1],
                                                   self.__ac_functions,
                                                   self.__fn_args_mask):
            ...
            self.__cache.append(prediction)
            prediction = ac_function(
                prediction, args.item()) if args else ac_function(prediction)
        return softmax(torch.mm(prediction, self.__weights[-1]) +
                       self.__biases[-1],
                       dim=1)

    def backward(self, inputs: Tensor, expected_output: Tensor,
                 predicted_output: Tensor) -> None:
        """
        ...
        """
        current_grad = (predicted_output -
                        expected_output) / expected_output.size()[0]
        ...
        for i in range(len(self.__weights) - 1, 0, -1):
            ...
            h_grad = current_grad @ (self.__weights[i].t() *
                                     self.__keep_prob[i].t())

            if self.__fn_args_mask[i - 1] is None:
                current_grad = derivatives[self.__ac_functions[i - 1]](
                    self.__cache[i - 1]) * h_grad
            else:
    ...
    def load_weights(self, weights, out_weights, biases, out_bias):
        self.__weights = ParameterList(
            [Parameter(W) for W in weights + [out_weights]])
        self.__biases = ParameterList(
            [Parameter(b) for b in biases + [out_bias]])
